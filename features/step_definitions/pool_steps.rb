...
Then(/^they should be able to (.*?) pools$/) do |action|
  if action == "add"
 expect(page).to have_selector("input[type=submit][value='Add to pool']")
  elsif action == "remove"
 expect(page).to have_selector("input[type=submit][value='Remove from pool']")
  end
end
...
Then(/^they should see it pending$/) do
